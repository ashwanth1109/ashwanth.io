...
At Crossover, during the month of December (2020), January and February (2021),
we were given some time off projects to do some research related to the usage of AWS services.

The idea was to come up with Engineering Outcomes that would affect the organization as a whole.
This was also to support a massive movement in the organization called "Ship Every Merge" which
was an idea for how developers across the org would build products and come up with a unifying
ideology. So, while this process was underway, we started picking different AWS services each week,
and to try and come up with technical decisions backed by data.

Along with my colleague, "Adrian Diaconu" (a senior software engineer in the company), I decided to
...
While the answer itself seems fairly straightforward, and was the decision we'd taken on our previous project, we now had the data to back it up.
